"""
Generate sales forecasts using Statistical + AI/ML models

This management command generates sales forecasts for school products at multiple levels:
- By School
- By Product
- By Shop Location
- By Category

Time horizons: 30 days, 90 days, 180 days, 365 days
"""
...
from dashboard.models import SalesForecast, DashboardCache
...
class Command(BaseCommand):
    help = 'Generate sales forecasts for school products using Statistical + AI/ML models'

    def add_arguments(self, parser):
        parser.add_argument(
            '--horizon',
            type=str,
            choices=['30d', '90d', '180d', '365d', 'all'],
            default='all',
        ...
        parser.add_argument(
            '--model',
            type=str,
            choices=['statistical', 'ml', 'hybrid', 'all'],
            default='hybrid',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('SALES FORECASTING ENGINE - Statistical + AI/ML Models'))
        self.stdout.write(self.style.SUCCESS('=' * 80))
        ...
        # Define horizons to process
        horizons = ['30d', '90d', '180d', '365d'] if horizon == 'all' else [horizon]
        levels = ['school', 'product', 'shop'] if level == 'all' else [level]

        for h in horizons:
            for l in levels:
                self.stdout.write(f"\n{self.style.WARNING(f'Processing: {l.upper()} - {h}')}")
                self.generate_forecasts(h, l, model_type, min_sales)

        self.stdout.write(self.style.SUCCESS('\n✓ Forecasting complete!'))

    def generate_forecasts(self, horizon, aggregation_level, model_type, min_sales):
        """Generate forecasts for a specific horizon and aggregation level"""
        ...
        for entity_name in sales_data['entity_name'].unique():
            ...
            if model_type == 'statistical' or model_type == 'hybrid':
    ...
    def get_historical_sales(self, aggregation_level):
        ...
        entity_fields = {
            ...
            'product': 'li.code',  # FIXED: Use SKU code to forecast each variation separately
        ...
        try:
            ...
            if PROPHET_AVAILABLE:
                try:
                    ...
                    # Instead of using make_future_dataframe which includes historical dates,
                    # we'll create a custom future dataframe starting from today
                    ...
                    self.stdout.write(self.style.WARNING(f'    Prophet failed: {prophet_error}, falling back...'))
            ...
            if days_with_sales / num_days >= 0.3 and STATS_AVAILABLE:  # At least 30% of days have sales
                try:
                    ...
                    forecast_dict = {}
                    # Use today's date for forward-looking forecasts instead of last historical date
            ...
            # This is more robust for school products that order occasionally
            forecast_dict = {}
            # Use today's date for forward-looking forecasts instead of last historical date
            start_date = datetime.now().date()
    ...
    def save_forecast(self, entity_name, horizon, aggregation_level, model_type, forecast_data, training_data):
        """Save forecast to database"""

        if forecast_data is None or 'forecasts' not in forecast_data:
            return

        forecast_id = f"{aggregation_level}_{entity_name.replace(' ', '_')}_{horizon}_{datetime.now().strftime('%Y%m%d')}"
        ...
        try:
            ...
            if len(training_data) >= 60:
                ...
                if test_forecast:
                    ...
                    # This makes more sense for intermittent demand anyway
        ...
        SalesForecast.objects.update_or_create(
            ...
            defaults={
                ...
                'model_type': model_type,
                'forecast_data': forecast_data['forecasts'],
